          ...
          {process.steps.map((step, index) => {
            ...
            return (
              ...
              >
                ...
                <div className="relative bg-slate-900 border border-gold-500/20 rounded-xl p-8 hover:border-gold-500/50 transition-colors duration-500 h-full flex flex-col items-center text-center">
                  {/* Number Badge */}
                  <div className="absolute top-4 right-4 text-xs font-mono text-gold-500/40 border border-gold-500/20 px-2 py-1 rounded">
                    {step.number}
                  </div>

                  {/* Icon */}
                  <div className="w-20 h-20 rounded-full bg-slate-950 border border-gold-500/30 flex items-center justify-center text-gold-400 mb-6 group-hover:scale-105 group-hover:bg-slate-900 transition-all duration-300 shadow-[0_0_15px_-3px_rgba(212,175,55,0.1)]">
                    <Icon className="w-8 h-8" />
                  </div>

                  {/* Content */}
                  <h3 className="font-serif text-xl font-bold text-white mb-4 group-hover:text-gold-200 transition-colors">{step.title}</h3>
                  <p className="text-sm text-slate-200 leading-relaxed opacity-90">{step.description}</p>
                </div>
              </motion.div>
            )
          })}
            ...
            {process.faq.map((item, index) => (
              ...
              >
                <button
                  ...
                  className="w-full px-6 py-5 flex items-center justify-between text-left hover:bg-slate-900/80 transition-colors focus-ring"
