...
const Navbar: React.FC<NavbarProps> = ({ content, common }) => {
  ...
  const navLinks = [
    ...
    { label: content.whyUs, href: "#expertise" }, // Updated ID to match WhyUs.tsx
        ...
        <div className="hidden lg:flex items-center gap-8">
          {navLinks.map((link) => (
            <a
              key={link.label}
              href={link.href}
              className={`text-sm font-medium tracking-wide transition-colors ${isScrolled ? 'text-slate-600 hover:text-gold-600' : 'text-slate-200 hover:text-white'
                }`}
            >
              {link.label}
            </a>
          ...
          <a
            href="#contact"
            className={`px-6 py-2.5 rounded-full text-sm font-semibold transition-all shadow-lg hover:shadow-xl hover:-translate-y-0.5 ${isScrolled
