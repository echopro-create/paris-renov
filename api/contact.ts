...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    const { data, error: resendError } = await resend.emails.send({
      ...
      replyTo: email,
      subject: `Nouveau devis - ${escapeHtml(name)}`,
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
          ...
          <table style="width: 100%; border-collapse: collapse;">
            ...
            <tr>
              ...
              <td style="padding: 12px;"><a href="mailto:${escapeHtml(email)}">${escapeHtml(email)}</a></td>
            </tr>
            <tr style="background: #f9f9f9;">
              <td style="padding: 12px; font-weight: bold;">Téléphone :</td>
              <td style="padding: 12px;"><a href="tel:${escapeHtml(phone)}">${escapeHtml(phone)}</a></td>
            </tr>
            <tr>
              <td style="padding: 12px; font-weight: bold;">Type de projet :</td>
              <td style="padding: 12px;">${escapeHtml(type || 'Non spécifié')}</td>
            </tr>
            <tr style="background: #f9f9f9;">
              <td style="padding: 12px; font-weight: bold; vertical-align: top;">Message :</td>
              <td style="padding: 12px;">${escapeHtml(message).replace(/\n/g, '<br>')}</td>
            </tr>
          </table>
